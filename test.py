...
if st.session_state.page == "graph":
    ...
    # --- Build base64 strings for the floating bar ---
    png_b64 = base64.b64encode(png_data).decode("utf-8")
    jpg_b64 = base64.b64encode(jpg_data).decode("utf-8")
    svg_b64 = base64.b64encode(svg_data).decode("utf-8")

    # --- Floating top-right download bar (Option B) ---
    st.markdown(f"""
    <style>
    .download-bar {{
        position: fixed;
        top: 16px;
        right: 80px;  /* adjust to sit left of Streamlit settings */
    ...
    <div class="download-bar">
        <a href="data:image/png;base64,{png_b64}" download="data_flow_graph.png">⬇️ PNG</a>
        <a href="data:image/jpeg;base64,{jpg_b64}" download="data_flow_graph.jpg">⬇️ JPG</a>
        <a href="data:image/svg+xml;base64,{svg_b64}" download="data_flow_graph.svg">⬇️ SVG</a>
    </div>
    """, unsafe_allow_html=True)
